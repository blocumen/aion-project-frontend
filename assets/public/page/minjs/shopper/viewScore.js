...
$('body').on('click', '#submitDateBetween', function() {
    ...
    if (minD == '' || maxD == '') {
        ...
        $.ajax({
            'url': '/shopper/bar/ajax',
            'type': 'get',
            'data': { minD: minD, maxD: maxD, barId: barId },
            'dataType': 'json',
...
function openViewScoreEditModel(scoreId) {
    //Calling Ajax
    $.ajax({
        'url': '/shopper/bar/score/detailById',
        'type': 'post',
        'dataType': 'json',
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (response.status === true) {
                ...
                setTimeout(function() {
                    window.location.reload() = "/shopper/bar/viewScore/" + barId + "/" + barName;
                }, 1000);
                swal({
                    title: 'Done!',
